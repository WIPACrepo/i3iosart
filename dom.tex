...
supernova event (Sec.~\ref{sect:SNDAQ}) \cite{IC3:supernova}.

DOMs transmit their data to computers in the ICL
over a twisted wire pair that also provides power (Sec.~\ref{sec:cable}).
...
to single photon detection (Sec.~\ref{sect:online:payloads}).
...
the DOM glass.  The wires are routed inside a customized cable, shown in Fig.~\ref{fig:domcable},
...
long penetrator assembly cable (Fig.~\ref{fig:domcable}).
...
The PMT high voltage generator is a custom encapsulated module (\#9730A) designed by
EMCO High Voltage (California).  The PMT high voltage
subsystem consists of a resistive 
...
Board interfaces to other boards as shown in Fig.~\ref{fig:domcomponents} and
