...
containing a downward-facing \SI{10}{''} diameter PMT
...
Separate circuit boards generate PMT high voltage, interface to the PMT pins~\cite{ICECUBE:PMT},
...
The PMT detects signals from neutrino events typically ranging over
energies from \unit[10]GeV to \unit[10]PeV and distances from a few meters to \unit[500]m
...
data for a period of \unit[1]s before sending the data up as a block, although the DOM
...
Its properties have been measured and described in Ref.~\cite{ICECUBE:PMT}.
...
with in-ice DOMs operated at a gain of $10^7$ (Sec.~\ref{sec:domcal}).
...
The ATWD recording interval is \unit[427]ns.  This is ideal for
...
Each DOM contains an LED Flasher Board, which is used for a
...
The pulse width parameter (0-127) controls width up to a maximum of \unit[70]{ns}~\cite{Aartsen:2013rt}; 
...
on brightness (B) and width configuration parameters.  Additional dynamic range is available
...
Approximately 5500 in-ice and IceTop DOMs were built, tested and
delivered to the South Pole. The DOMs satisfied stringent requirements, needed to ensure reliable operation in the deep ice for at least 20 years. As hot-water drilling was the principal
...
1000 DOMs produced and tested. Finally, Stage 3 involved procurements,
