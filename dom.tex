...
depending on hit rate.  Separately, the PMT hit rate is recorded by
...
(LC) where
nearest or next-to-nearest neighbors are hit within a common \unit[1]$\mu$s
time window. The signals are forwarded from one DOM to the next
through the dedicated wiring; the span of the forwarding is software-configurable and is currently set to two, i.e. a DOM signals its neighbor and next-to-nearest neighbor DOMs in both up and down directions along the string. Local coincidence hits often have complex PMT waveforms
...
Approximately 5800 in-ice and IceTop DOMs were built and tested, with
