...
maximum brightness and the mean time at the DOM above the flashing DOM
is recorded. Given the vertical spacing of 17~m and the group index of
refraction of  1.356 in ice at 400~nm, the expected light travel time
from the flashing DOM to the DOM above is 77~ns. The mean light travel
time to the DOM above for all flashing DOMs in ice is shown in
Figure~\ref{fig:flashertiming}, and it agrees with the prediction of
77~ns within the timing uncertainty of 3~ns. Muons are also used to
verify the time calibration: a track is fitted to a muon with the DOM
under study omitted from the track, and the time residual is
calculated as the difference between the expected hit time on the DOM
from the fitted track and the actual hit time. The difference between
the flasher time between adjacent DOMs and the muon time residual is
plotted in Figure~\ref{fig:flashertiming} and is consistent with zero,
with a laerger scatter due to the uncertainty of muon track position
in the ice.

\begin{figure}[!h]
  \captionsetup[subfigure]{labelformat=empty}
  \centering
  \subfloat[]{\includegraphics[width=0.5\textwidth]{graphics/dom/rapcal/Allmean_corrected.pdf}}
  \subfloat[]{\includegraphics[width=0.5\textwidth]{graphics/dom/rapcal/Muonflashermean.pdf}}
  \caption{Left: Distribution of the mean light travel time from
    flashing DOMs to the DOM above in ice. Right: Difference between
   light travel time between DOMs calculated with muons and
   flashers. FIXME - figures need to be updated}
...
effect on the physics data is small.
