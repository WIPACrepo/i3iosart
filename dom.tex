...
can store $\mathcal{O}(\SI{10}{\second})$ of data before overflowing,
...
supernova event (see Sec.~\ref{sec:online:supernova}).

DOMs transmit their data to computers in the IceCube Laboratory building
using a twisted wire pair that also provides power (Sec.~\ref{sec:cablesystems}).
...
the surface computers to communicate with two DOMs in parallel.  The terminated DOM is deployed
\unit[17]m or \unit[10]m below the unterminated one and therefore includes a correspondingly
long penetrator assembly cable (bottom of Fig.~\ref{fig:domcable}).

The entire penetrator assembly was designed and produced by SEACON
(California).  The part that seals against the DOM glass is similar to a
...
The PMT is specified by Hamamatsu for the wavelength range \unit[300]nm--\unit[650]nm, with peak quantum efficiency around 25\% (34\% for HQE)
near \unit[390]nm.  It features a box-and-line dynode chain with 10 stages,
...
To reduce effects of the ambient magnetic field (\unit[550]mG, $17^\circ$
...
\item{Digitization of the PMT waveforms, using a custom ASIC (ATWD: Analog
  Transient Waveform Digitizer) and a continuously sampling ADC.}
...
\item{Communicating with the data acquisition system on the surface.}
...
also weaker, and the information is captured in the \unit[40]Msps ADC.  This ADC samples continuously, and
the FPGA is programmed to save an interval of \unit[6.4]$\mu$s after the launch. Its preamplifier provies a dynamic 
...
%value 127) and W is the width setting (maximum value 127).
...
and local coincidence validation.  Optical sensitivity was measured using
a laser system FIX ME FIX ME.  A typical FAT temperature and testing cycle
is shown in Fig.~\ref{fig:fat_cycle}. The initial pass rate for DOMs during FAT
...
% FIX ME DFL TEMP, LUX, PASS RATE, and FATE OF FAILING DOMS
...
approximately 5500 DOMs shipped to Pole, about 30 (0.5\%) were shipped back
to the U.S.~after failing on-ice testing.    
