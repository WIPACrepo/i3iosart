...
of four DOMs, which are serviced by two wire pairs from the surface that carry power and
communications.  The ``T'' labels show where electrical termination (\unit[140]$\Omega$) is
...
the surface computers to communicate with two DOMs in parallel.  The
...
also weaker, and the information is captured in the \unit[40]Msps fADC.
...
was approximately 90\%, with failing DOMs retested and/or reworked as
needed.  After the successful completion of FAT, DOM deployment harnesses
were attached and the DOMs individually packed. 

% FIX ME PASS RATE, and FATE OF FAILING DOMS
