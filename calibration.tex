...
\begin{figure}[!h]
  \captionsetup[subfigure]{labelformat=empty}
  \centering
  \subfloat[]{\includegraphics[width=0.5\textwidth]{graphics/dom/rapcal/flashmean.pdf}}
...
absence of a photon source external to the DOM. The average total in-ice hit rate of
DOMs with standard PMTs is \SI{560}{\hertz} and \SI{780}{\hertz} for high
quantum efficiency DOMs. The contribution from muons, estimated as the
in-ice HLC hit rate, decreases with depth from 25 Hz to 5 Hz.  The dark noise is composed of two major 
components.  First, electronic noise, field and thermionic emission, and
radioactive decays \cite{hamamatsu_pmt} in the DOM material form 
the uncorrelated (Poissonian) noise pulses with a rate between
\SI{230}{\hertz} and \SI{250}{\hertz}, following Richardson's law \cite{richardson1901} for
thermal emission.  The second component is the so-called correlated
noise, with a pulse rate from \SI{280}{\hertz} to \SI{340}{\hertz}, that
manifests itself as an overabundance of short time intervals between hits
in a single DOM compared to the Poisson expectation (Fig.~\ref{fig:darknoise_deltaT}).
...
good agreement with results reported in Ref.~\cite{meyer_noise},
but an unambiguous physical explanation is still to be determined.  One
candidate for the source of the process is luminescence of the glass
triggered by radioactive decay of elements such as thorium and uranium
contained in the glass of the DOM.  
...
rate decay is most pronounced when selecting only strings
