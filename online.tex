...
each DOM in turn; only one DOM (out of eight) can transmit at a time.  Communication is
...
hits.  Next, triplets of HLC pairs within a time window $T_{\mathrm{max}}$
are formed.  The geometry of each triplet formed must satisfy track-like
conditions: the obtuse inner angle $\alpha$ of the triangle formed by the earliest
DOM in each pair (see Fig.~\ref{fig:slop}) must be larger than $\alpha_{\mathrm{min}}$, and the
...
Request daemon to collect raw data from significant transient events. Presently, the HitSpool Request System has three clients;
...
components.  The associated JavaScript Object Notation (JSON) data are forwarded to LiveControl using ZeroMQ and queued internally
...
Nagios monitoring software tracks the SPS hardware status and provides the link
