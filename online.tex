...
analyses are changed at most once a year (typically in May), indicating the start of a new
``physics run''.   
...
Sec.~\ref{sect:online:evbuilder}) is about 240~Mbps in each direction, while
...
Fig.~\ref{fig:clock_fanout}. The 10 MHz output, a 1 Hz output, and a serial time string indicating
...
multiplexes the communications signaling on top of this DC level. 
...
subtriggers as needed.  The trigger requests are used by the Event Builder
...
hits to a local ``HitSpool'' on-disk cache, as well as queuing them in an
...
end is called Omicron and the back end is the Sender. Omicron controls all of the connected DOMs, forwarding any
non-physics data (calibration, monitoring) to its downstream consumers and sorting the hits from all
...
are close in time ($\Delta t < T_{\mathrm{prox}}$), removing many muon
hits.  For all parameters, the trigger algorithm considers the time and
position of the first hit within each HLC pair.  Next, triplets of HLC
pairs within a time window $T_{\mathrm{max}}$ 
are formed.  The geometry of each triplet formed must satisfy track-like
conditions: the obtuse inner angle $\alpha$ of the triangle formed by the
HLC pairs (see Fig.~\ref{fig:slop}) must be larger than $\alpha_{\mathrm{min}}$, and the
...
DOM set involved in an in-ice trigger, the readout windows are appended at the end of the trigger
...
(see Sec.~\ref{sect:online:filter}).
...
interval and forward them to a single ``sender'' daemon
that bundles them to be transferred to the Northern Hemisphere for further analysis.
...
\begin{tabularx}{\textwidth}{lcXXXX}
  ...
  \midrule
  SNDAQ & $7.65 \ge \xi < 10$\ 
  ($\xi \ge 10$) & $[-30\,\mathrm{s},+60\,\mathrm{s}]$ ($[\pm250
...
satellite bandwidth allocations (about 100 GB/day).  This treatment
...
components include: 
\begin{enumerate}
\item DAQ Dispatch, a process to pickup event data from the DAQ data cache and forward to the PFServer components.
...
A number of other filters are employed for more specialized searches, as well as for minimum bias selections.
...
four primary tasks: data pickup, archival, satellite transmission, and
...
were formerly written to Linear Tape Open (LTO) tapes but are now written to 4 TB disks.
...
forwarded to LiveControl using ZeroMQ and queued internally 
...
\begin{figure}[!ht]
 ...
 \caption{Detector uptime since the start of full-detector
